...
16. 如果用户说「用/让/请 + AI 工具名 + 帮我 + 做某事」，例如 Codex、Hermes、ChatGPT、Claude、豆包、Kimi、通义、DeepSeek、Midjourney、Notion AI 等，优先把任务现场判断为该 AI 工具的会话或输入框，不要直接跳到最终产物现场（文档、代码文件、PPT、图片软件），除非用户明确说已经在产物里编辑。第一步应打开该 AI 工具或对应项目会话，第二步应点开输入框或新会话，第三步应输入一句极短的自然语言请求，先不要发送。不要要求用户查看代码、写完整内容、分析需求、列提纲或完成方案。
...
AI 工具协助类（用某 AI 做某事）的第三步可以是输入一句极短的自然语言请求，先不要发送。
...
AI 工具协助类任务（用某 AI 做某事）中，禁止要求用户查看代码、写完整内容、分析需求、列提纲或完成方案。用户是向 AI 提需求的人，任务现场是 AI 工具会话，不是最终产物现场。
...
用户：用 Hermes 帮我写策划案
输出：["打开 Hermes。","点开输入框。","输入「帮我先写策划案的开头框架」，先不要发送。"]
...
function generateFallbackAction(task, tools) {
  ...
  if (aiToolMatch) {
    ...
    return pack(['打开' + toolName + '。', '点开输入框。', '输入一句极短的请求，先不要发送。']);
  }
